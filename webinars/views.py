...
@login_required
def resend_zoom_invite(request, attendee_id):
    """Resend Zoom invitation email to an attendee."""
    if request.method != 'POST':
        return JsonResponse({'success': False, 'message': 'Invalid request method'}, status=405)
    
    attendee = get_object_or_404(Attendee, pk=attendee_id, deleted_at=None)
    
    # Check if attendee is registered in Zoom
    if not attendee.zoom_registrant_id:
        return JsonResponse({
            'success': False, 
            'message': 'Attendee is not registered in Zoom'
        }, status=400)
    ...
    try:
        ...
        zoom_service = ZoomService()
        result = zoom_service.resend_invite_email(
            attendee.webinar_date.zoom_meeting_id,
            attendee.zoom_registrant_id
        )
        
        if result['success']:
            messages.success(request, f'Successfully resent Zoom invitation to {attendee.email}')
            return JsonResponse({
                'success': True, 
                'message': f'Successfully resent Zoom invitation to {attendee.email}'
            })
        else:
            messages.error(request, f'Failed to resend Zoom invitation: {result["error"]}')
            return JsonResponse({
                'success': False, 
                'message': f'Failed to resend invitation: {result["error"]}'
            }, status=400)
            
    except Exception as e:
        error_msg = f"Error resending Zoom invitation: {str(e)}"
        messages.error(request, error_msg)
        return JsonResponse({
            'success': False, 
            'message': error_msg
